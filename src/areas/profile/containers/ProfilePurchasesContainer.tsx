...
function ProfilePurchasesContainer() {
  ...
  return (
    <QueryContainer action={action} mapping={mapLotsLists}>
      {payload => {
        const won = payload.items.filter(item => [LotTradeStatus.AWAITING_PAYMENT, LotTradeStatus.PAID, LotTradeStatus.AWAITING_SHIPMENT].includes(item.tradeStatus))
        const delivering = payload.items.filter(item => item.tradeStatus === LotTradeStatus.DELIVERY)
